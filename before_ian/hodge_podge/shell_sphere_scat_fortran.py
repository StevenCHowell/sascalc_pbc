...
def pointsInSphere():
	coor=[]
	file=open('51balls.txt','r').readlines()
	for i in range(len(file)):
		if (i>0):
			lin=string.split(file[i])
			x=locale.atof(lin[1])
			y=locale.atof(lin[2])
			z=locale.atof(lin[3])
			coor.append([x,y,z])		 
...
#			sys.exit(1)
	...
	#coor=1*numpy.array(pointsOnSphere(nresidues))
	coor=1*numpy.array(pointsInSphere())
	resmol.setCoor(coor)
	resmol.writepdb('51sphere.pdb','w')
	...
	outfile=open('51sqfaad2o'+str(nqvectors)+'.dat','w')
