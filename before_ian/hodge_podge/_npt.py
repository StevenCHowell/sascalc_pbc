...
def mcvol(coor,ran,natoms,beta,boxl,oldenergy,vmax):
	...
	for i in xrange(natoms):
		xcoor[i]=xcoor[i]*boxn/boxl
		ycoor[i]=ycoor[i]*boxn/boxl
		zcoor[i]=zcoor[i]*boxn/boxl
		
	enn=calcenergy2(coor,natoms,boxn)
	eno=oldenergy	
	...
	arg=-beta*((enn-eno)+p*(vn-vo) + -(natoms+1)*math.log(vn/vo)/beta)

	#print 'arg = ',arg
	if arg<100:
		if(random.random()>math.exp(arg)):
			for i in xrange(natoms):
				xcoor[i]=xcoor[i]*boxl/boxn
				ycoor[i]=ycoor[i]*boxl/boxn
				zcoor[i]=zcoor[i]*boxl/boxn
		
			coor[:,0]=xcoor ; coor[:,1]=ycoor ; coor[:,2]=zcoor
		else:
			boxl=boxn
			result=1

	return result,enn,boxl
...
def nptmc(nmolecules,temperature,pressure,eta_goal,nsteps,nbins,runfilename):
	...
	for i in xrange(nsteps):
		...
		if ran < nmolecules:
			...
			oboxl=boxl
			result,oldenergy,boxl=mcvol(coor,ran,natoms,beta,boxl,oldenergy,vmax)
			if result==1:
				writexyz(coor,runfilename,'a')
				accvol+=1
				print 'old boxl = ',oboxl
				print 'new boxl = ',boxl
				boxsum=boxsum+boxl
